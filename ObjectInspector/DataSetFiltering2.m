classdef DataSetFiltering2 < handle
    methods 
...
%             DATA2 = [DateStamp;DATA2];
            ...
            try
                ...
                disp(['Path: ',FileName]);
                warning('Problem writing csv file'); 
            end
        end
    ...
    methods (Hidden = true)
        ...
        function writecsv(obj,FileName,DATA)
            ...
            if fid == -1
               uiwait(msgbox({  'You can''t write to the directory.'; ...
                                'Please check you have the premission'; ...
            ...
            for i = 1:x
                for j = 1:y
                    Entry = DATA{i,j};
                    n = findstr(Entry,'Diffused');
                    if not(isempty(n))
                        x = 1;
                    end
                    warning off
                    Entry = strrep(Entry,carriage_return,' ');
                    warning on
                    switch class(Entry)
                        case 'char'
                            %do nothing
                ...
                fprintf(fid, [strrep(Line,'\','/'),'\n'], y); %The replace function was to remove invalid sequences. 
            end
            fclose(fid);
        end
    end
