...
%             icon = ind2rgb(X,map);
            ...
            filename = fullfile( matlabroot,'toolbox','matlab','icons','plottoolson'); 
            load(filename)
            hpt = uipushtool(ht,    'CData',cdata, ...
                                    'TooltipString','plot parameters', ...
                                    'ClickedCallback',@obj.PlotParameter);
...
%             ndata= data(68580,4);
        ...
        function PlotParameter(varargin)
            obj = varargin{1}
            disp('hello')
            
            obj = plotDATASET('DATASET',obj.DATASET);
            ObjectInspector(obj)
        end
    end
end