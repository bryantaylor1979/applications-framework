classdef plotSpectrum <     handle
    properties (SetObservable = true, GetObservable = false, Hidden = true)
        DATASET
    end
    properties (SetObservable = true)
        Mode = 'Ref_Lum'; %Ref_Lum or Manual
        Title = 'Transmission spectrum';
        spectrumSTD = '1964_full';   %This changes the axis colour visualisation                
    ...
    methods
        function Example(obj)
            ...
            close all
            clear classes
            spectrumName = '3200BB_reference';
            READ_OBJ = getSpectrumFromFile_LE7('filename', spectrumName)
            READ_OBJ.RUN();
            DATASET = READ_OBJ.DATASET;
            obj = plotSpectrum( 'DATASET',  DATASET, ...
                                'Title',    spectrumName);
            
            %% UPDATE
            spectrumName = 'A_reference';
            READ_OBJ.filename = spectrumName;
            READ_OBJ.RUN();
            DATASET = READ_OBJ.DATASET;
            obj.DATASET = DATASET;
            obj.Title = spectrumName;
            
            
            
            %% plot A
            obj = plotSpectrum( 'DATASET',      DATASET, ...
                                'spectrumSTD',  'Stiles_10');         
            
            %% plot from red tide
            objRT = RedTide(   'integrationTime', 50);
            obj = plotSpectrum( 'wavelengths',  objRT.wavelengths, ...
                                'spectralData', objRT.spectralData);
            
            %% update plot from red tide
            obj.spectralData =  objRT.spectralData;
    ...
    methods (Hidden = true)
        function obj = plotSpectrum(varargin)
            % Read in dynamic inputs.
            x = size(varargin,2);
            for i = 1:2:x
                obj.(varargin{i}) = varargin{i+1};
            end
            % plot
            if strcmpi(obj.Mode,'Ref_Lum')
                
                obj.plot(   obj.DATASET.Wavelength,    obj.DATASET.Transmission);
            else
                obj.plot(   obj.DATASET.Wavelength,    obj.DATASET.Transmission);
            end
            obj.handles.listeners.PostSet = obj.addlistener('DATASET','PostSet',@obj.SET); 
            obj.handles.listeners.PostSet = obj.addlistener('Title','PostSet',@obj.TitleUpdate); 
            
            %%
            axes(obj.handles.axes);
            XLim = get(obj.handles.axis,'XLim');
            YLim = get(obj.handles.axis,'YLim');
            YPos = YLim(1) + (YLim(2)-YLim(1))*0.5;
            
            %%
            [cct,duv,lux] = obj.GetSpectrumCalcs();
            
            string = {  ['CT:         ',num2str(round(cct)),'K']; ...
                        ['DeltaUV: ' ,  num2str(round(duv*10000)/10000)]; ...
                        ['Lux:        ',num2str(round(lux*10)/10)]};
            obj.handles.text = text(XLim(1), YPos, string);
            YPos = YLim(1) + (YLim(2)-YLim(1))*0.91;
            XPos = XLim(1) + (XLim(2)-XLim(1))*0.04;
            set(obj.handles.text,'Position',[XPos, YPos,0]);
        end
        function plot(obj,wavelengths,spectralData)
            
            % Plot diffuser transmission spectrum
            BasedColour = [1,0,0];
            
            obj.handles.figure = figure(    'Visible',      'off', ...
                                            'NumberTitle',  'off', ...
                                            'Name',         'Spectrum');
            obj.handles.line = plot(wavelengths,spectralData);
            obj.handles.axes = gca;
            
            Pos1 = 0.13;  %0.13 Left
            Pos2 = 0.15;  %0.11 Bottom
            Pos3 = 0.775; %0.775 %width
            Pos4 = 0.775; %0.815 %height
            set(obj.handles.axes,'Position', [Pos1 Pos2 Pos3 Pos4])
            
            %%
            obj.handles.title = title(obj.Title);
            set(obj.handles.title,'Interpreter','none');
            ylabel('Transmission (%)');
            obj.handles.axis = gca;
            obj.spectrumLabel(gca) 
            
            xlabel('Wavelength');
            
            set(obj.handles.line,'Color',BasedColour);
            set(obj.handles.axes,'XLim',[min(wavelengths),max(wavelengths)]);

            drawnow;
            set(obj.handles.figure,'Visible','on');
        end
        function [cct,duv,lux] = GetSpectrumCalcs(obj)
            result =  [obj.DATASET.Wavelength,obj.DATASET.Transmission];
            if min(obj.DATASET.Wavelength) > 380
                result = [380, 0; result];
            end  
            [XYZ2deg,ispd,lux,xyz2deg,cct,duv,Ra,R] = pspectro(result);            
        end
        function varargout = spectrumLabel(obj,hAxesTarget)
            ...
            [lambda, RGB] = obj.createSpectrum(obj.spectrumSTD);
            axes(hAxesSpectrum);
        ...
        function [lambda, RGB] = createSpectrum(obj,matchingFcn)
            ...
            [lambda, xFcn, yFcn, zFcn] = obj.colorMatchFcn(matchingFcn);
        ...
        function SET(varargin)
            %%
            obj = varargin{1};
            set(obj.handles.line,'YDATA',obj.DATASET.Transmission);
            set(obj.handles.line,'XDATA',obj.DATASET.Wavelength);
            
            %%
            [cct,duv,lux] = GetSpectrumCalcs(obj);
            ...
            YPos = YLim(1) + (YLim(2)-YLim(1))*0.91;
            XPos = XLim(1) + (XLim(2)-XLim(1))*0.04;
            set(obj.handles.text,'Position',[XPos, YPos,0],'String',string);

        end
        function TitleUpdate(varargin)
            %%
            obj = varargin{1};
            set(obj.handles.title,'String',obj.Title);
        end
    end
